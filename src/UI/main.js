...
$(function () {
  ...
  var missedData = []; //Array to insert missed tiles data that were to be sent
  ...
  function initializeMap() {
    ...
    mapboxgl.accessToken =
      "pk.eyJ1IjoibWVyY3VyeXJlaWduIiwiYSI6ImNsbnZ3YnF2MDAydHgybmp5MWRlZGJ6dGwifQ.cU-vFZvjlnWDngTGd7410w";
  ...
  async function startDownloading() {
    ...
    var toastHTML =
      '<div class="sometexts" style="flex-direction : column">Starting download! at : ' +
      showTime +
      '<div class="progress"><div class="indeterminate"></div></div></div>';
    ...
    var iterator = async.eachLimit(
      ...
      function (item, done) {
        ...
        var request = $.ajax({
          ...
          .done(function (data) {
            ///this is where tile is being taken from
            ...
            if (data.code == 200) {
              ...
              logItem(
                ...
                item.y,
                item.z,
                data.code + " Error downloading " + message
              );
      ...
      async function (err) {
        ...
        logItemRaw(
          "\nTotal elapsed time: " +
            new Date(finishTime - startTime).getSeconds() +
            " seconds"
        );
        ...
        if (validateDownload(data)) {
          M.toast({
            html: "Finished download! at " + showdate,
            displayLength: 7000,
            classes: "success",
          });
  ...
  function validateDownload(data) {
    ...
    if (missedRequest.length > 0) {
      var toastHTML =
        ...
        missedRequest.length +
        " out of " +
        requests.length +
        " had problems Retry?" +
        '<button id="retry"  class="btn-flat toast-action"> Yes </button><button id="noretry" class="btn-flat toast-action"> No </button>';
    ...
    async function checkfile(data) {
      ...
      var request = await $.ajax({
        ...
        if (data.missFiles.length > 0) {
          var toastHTML =
            "Download complications, " +
            data.missTiles.length +
            " are missing Restart download?" +
            '<button id="retry"  class="btn-flat toast-action"> Yes </button><button id="noretry" class="btn-flat toast-action"> No </button>';
  ...
  function retryDownload(oldData) {
    ...
    var toastHTML =
      '<div class="sometexts" style="flex-direction : column">Retry download! at : ' +
      showTime +
      '<div class="progress"><div class="indeterminate"></div></div></div>';
          ...
          .done(function (data) {
            ...
            if (data.code == 200) {
              ...
              logItem(
                ...
                item.y,
                item.z,
                data.code + " Error downloading tile " + message
              );
